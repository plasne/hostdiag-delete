...
function listEntities(nextPartitionKey, nextRowKey) {

    const PARTITION_KEY = ":002Fsubscriptions:002F83e686f6:002D963b:002D4e64:002Dbff4:002D99dc369cb4ec:002FresourceGroups:002F5359af6b:002D166f:002D42b0:002Dbf69:002D1434011bcf46:002Fproviders:002FMicrosoft:002ECompute:002FvirtualMachines:002Fpelasne:002Dvm1"

    return new Promise((resolve, reject) => {

        // log start
        if (PARTITION_KEY) {
            logger.log("debug", `getting another batch of entities by partition "${PARTITION_KEY}"...`);
        } else {
            logger.log("debug", `getting another batch of entities...`);
        }
        ...
        const options = {
            method: "GET",
            agent: agent,
            url: `https://myaccount.table.core.windows.net/mytable(PartitionKey='${PARTITION_KEY}')${(STORAGE_SAS) ? STORAGE_SAS + "&" : "?"}$select=RowKey`,
            headers: {
