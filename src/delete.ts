...
if (!STORAGE_TABLE) throw new Error("You must specify STORAGE_TABLE in either .env or command line.");
if (!SUBSCRIPTION_ID) throw new Error("You must specify SUBSCRIPTION_ID in either .env or command line.");
if (!RESOURCE_GROUP) throw new Error("You must specify RESOURCE_GROUP in either .env or command line.");
if (!HOSTNAME) throw new Error("You must specify HOSTNAME in either .env or command line.");
...
// define the return object format
interface Entity {
    RowKey:      string,
    ".metadata": string
}
    ...
    if (MODE.toLowerCase() === "delete") {
        ...
        const producer = () => {
            if (buffer.length > 0) {
                ...
                if (!rowKey) throw new Error("buffer could not pop().");
                const deletion: Promise<any> = table.delete(PARTITION_KEY, rowKey).then(() => {
                    count++;
                }).catch(error => {
